...
class Mongodb extends Db{
    ...
    public function connect($config='',$linkNum=0) {
        if ( !isset($this->linkID[$linkNum]) ) {
            ...
            try{
                ...
                $this->_dbName=$config['database'];//默认没有选择数据库
     ...
     * 执行aggregate
     * @access public
     * @param array $aggregate  指令
     * @return array
     */
    public function aggregate($aggregate=array()) {
        N('db_write',1);
        $this->queryStr = 'aggregate:'.json_encode($aggregate);
        // 记录开始执行时间
        G('queryStartTime');
        $result   = $this->_collection->aggregate($aggregate);
        $this->debug();
        if(!$result['ok']) {
            E($result['errmsg']);
        }
